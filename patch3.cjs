...
// Find the index of mappedRows.push(newRow); inside handleComplete
let startIndex = code.indexOf(`mappedRows.push(newRow);`);
if (startIndex !== -1) {
    let replacement = `                    // Cleanup: If a group rate is missing/blank for this row, remove its wage type, valid from, and salary code
    ...
    code = code.replace(`mappedRows.push(newRow);`, replacement);
    
    // Also replace the ALL_WAGE_SALARY_VALID_FROM to add ALL_EMPLOYEE_GROUPS_RATES_VALID_FROM before it
    code = code.replace(
